...
export class PDFMerger {
	...
	async add(input, pages) {
		...
		if (typeof pages === "undefined" || pages === null || pages === "all") {
			...
			// e.g. [2,3,6] or ["2","3","6"]
			const pagesAsNumbers = pages.map((p) =>
				typeof p === "string" ? parseInt(p.trim()) : p,
			);
	...
	async _getInputAsUint8Array(input) {
		...
		if (input instanceof URL) {
			if (typeof fetch === "undefined") {
				throw new Error(
					"fetch is not defined. You need to use a polyfill for this to work.",
				);
			}
		...
		let errorMsg = `pdf-input must be of type ${allowedTypes.join(
			", ",
		...
		if (typeof input === "string" || input instanceof String) {
			try {
				...
				try {
					...
					throw new Error(
						`The provided string "${input}" is neither a valid file-path nor a valid URL!`,
					);
				}
			}
		}
